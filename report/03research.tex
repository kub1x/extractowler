...
implementation of {\tt ConfigurationFactory} class.  In
...
and usage of crOWLer. Configuration defined using this structure specifies all
...
FullCrawler}, {\tt Crowler} form the crawling process loop. In this loop {\tt
FullCrawler} fetches the source web pages and passes them one by one to the
{\tt Crowler}. The {\tt NextPageResolver}, which defines list of pages to be
crawled is structure implemented within the configuration and thus is specific
for given problem instance. Results are stored in the outer loop after each
scraped page. 
...
XXX Diret GET (POST) calls no javascript -> WebDriver

XXX Reference to uc4  -- javascript pagination (ajax) wouldn't work at all in original

XXX fix following paragraph after previous is written 
...
It consists of webserver and backend service. The webserver offers frontend for
