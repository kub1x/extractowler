...
In this chapter we will describe the research made on existing solutions for
...
obsolete. Its structure is, however, based on programmers imagination an
...
simply change its content frequently (e.g. news pages, forums, user content
...
data from the web, but it still have its challenges to overtake. 
...
second part create {\em district} object with its label (found in third table
...
offers frontend for configuring the crawling process. The application ten
follows the configuration scraping the data and storing results while using the
backend handle downloading. Strigil strongly focusses on the download process.
...
will be closely analyzed in next chapter \ref[chap:design]. 
...
and long running tasks. Its complicated architecture and installation process
prevents it from being effectively used in occasional simple, yet non trivial,
scraping tasks. 

Moreover its download system fetches only the raw HTML data (XXX like the
...
The functionality of IDE is, however, linked with its layout
...
procedures following the SOWLs workflow. 
