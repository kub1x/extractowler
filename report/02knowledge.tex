...
Open Data cloud diagram\cite[lod]. The visualisation
...
\caption/f The Linking Open Data cloud
diagram~\urlnote{http://lod-cloud.net/versions/2014-08-30/lod-cloud_colored.svg}
\endinsert
...
literally specify address of resource and in many cases can be directly
...
defined in the specification and slightly described in appendix
\ref[app:rdfsvocab] allow us to specify class hierarchy, properties with domain
...
result we obtain such a nodes that form a subgraph of the original graph that
...
RDF/XML is one of formats into which we can serialize our RDF data~\cite[wiki:rdfxml] . It is a
regular XML document containing elements and attributes from the RDF(S)
vocabulary. RDF/XML is one of the most common formats for RDF data
serialization. \ref[app:rdfxml]
...
datatypes~\cite[w3c:turtle].  It's syntax suits more naturally to RDF data as
