...
if ($.browser && $.browser.opera && opera.postError) { console = window.console = { log : function(){opera.postError(arguments); } }; }
...
jOWL.NS = function(prefix, URI){
	...
	if(arguments.length == 1){
		...
		for(var i=0;i<attr.length;i++){
			...
			if(nn.length == 2){
				if(attr[i].value == jOWL.NS.owl.URI){ jOWL.NS.owl.prefix = nn[1];}
				else if(attr[i].value == jOWL.NS.rdf.URI){ jOWL.NS.rdf.prefix = nn[1];}
				else if(attr[i].value == jOWL.NS.rdfs.URI){ jOWL.NS.rdfs.prefix = nn[1];}
				else if(attr[i].value == jOWL.NS.xsd.URI){ jOWL.NS.xsd.prefix = nn[1];}
				else { jOWL.NS(nn[1], attr[i].value);}
			}
		}
...
$.fn.extend({
	...
	xmlAttr : function(nodeName){
		...
		for(var i =0;i<t.length;i++){
			if(t[i].nodeName == nodeName){ return t[i].value;}
		}
	},
...
jOWL.Xpath = function(selector, elem){
  if(!jOWL.document) { throw "jOWL.document is null. Did you call jOWL.load()? Try to use callback in second parameter ;) "; }
	var node = null;
	...
	if($.browser && $.browser.msie){ return $($.makeArray(arr));} return $(arr); //this is needed for IE, it returns a length of 1 on empty node array
};
...
jOWL.load = function(path, callback, options){
	var that = this;
	if($.browser && $.browser.msie && location.toString().indexOf('file') === 0){ //IE won't load local xml files otherwise
...
jOWL.parse = function(doc, options){
	...
	else if(doc.documentElement.nodeName == 'parsererror'){jOWL.options.onParseError(doc.documentElement.firstChild.value); return false;}
...
jOWL.toString = function(xmlNode){
	...
	if($.browser && $.browser.msie){ return xmlNode.xml;}
...
jOWL.create = function(namespace, name, document){
	...
	if($.browser && $.browser.msie){ el.node = doc.createNode(1, namespace(name), namespace());}
